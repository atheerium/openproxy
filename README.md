...
CipherRoute runs as one binary on `127.0.0.1:4623`. Point any tool that speaks the OpenAI Chat Completions API at it (Claude Code, Codex, Cursor, Cline, OpenClaw, Copilot, ...) and CipherRoute:
...
CipherRoute is built to be driven by AI agents (Devin, Claude Code, Codex, Cursor, OpenClaw, …) end-to-end — install, init, configure, and verify without any browser interaction.
...
A combo is an ordered list of models. CipherRoute tries them in order, falling back when one is rate-limited or errors. Models are addressed as `<provider-prefix>/<model-id>`:
...
- **[RTK](https://github.com/rtk-ai/rtk)** — token compression pipeline. CipherRoute's `tool_result` compression is a port.
