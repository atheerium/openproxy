---
name: cipherroute
description: Install, initialize, and operate CipherRoute from the CLI — either guiding a human through setup or driving it fully autonomously as an agent. Use whenever the user asks to install the cipherroute binary, start the local AI router on 127.0.0.1:4623, configure providers / combos / keys, or wire an AI coding CLI (Claude Code, Codex, Cursor, Cline, OpenClaw, Copilot, …) into CipherRoute.
---

# cipherroute — install & operate from the CLI

[CipherRoute](https://github.com/quangdang46/cipherroute) is a single-binary AI router that exposes an OpenAI-compatible API on `127.0.0.1:4623` and fans out to 40+ providers with auto-fallback. This skill walks an agent through:

1. Installing or upgrading the `cipherroute` binary
...
test -f "$HOME/.cipherroute/db.json" && echo "data dir already provisioned" || true
```

If a binary is already on PATH, jump to step 2. Run `cipherroute doctor` early — it catches half-installed states.
...
Drops the binary at `~/.local/bin/cipherroute`. Idempotent (locked via `/tmp/cipherroute-install.lock.d`). Useful flags:
...
```bash
curl -fsSL "https://raw.githubusercontent.com/quangdang46/cipherroute/main/install.sh" \
...
Drops `cipherroute.exe` at `$env:USERPROFILE\.local\bin`. Pass flags by downloading first:
...
`cipherroute server init` creates an empty `db.json` at `$DATA_DIR` (default `~/.cipherroute/`) and emits **one** fresh admin API key — shown exactly once.
...
mkdir -p "$HOME/.cipherroute"
printf '%s\n' "$APIKEY" > "$HOME/.cipherroute/admin.key"
chmod 600 "$HOME/.cipherroute/admin.key"
...
cipherroute auth reset-password --show
...
cipherroute server start --detach --no-open --port 4624   # alt port
```
...
```bash
cipherroute schema list                  # available resource kinds
cipherroute schema show provider         # JSON shape
cipherroute schema example provider      # ready-to-edit example
```
...
cipherroute --robot provider apply --from-file /tmp/providers.json
# Or, with --prune to make the file authoritative:
# cipherroute --robot provider apply --from-file /tmp/providers.json --prune
```
...
```bash
cipherroute combo create my-stack \
...
OAuth providers (`cipherroute provider oauth …`) require a one-time browser dance. In a headless / agent context, prefer:
...
The bearer is the admin key captured in step 2, or any key minted via `cipherroute key add`. CipherRoute also has a `tool` subcommand (`cipherroute tool …`) that can apply these settings programmatically — run `cipherroute tool --help` to see the matrix of supported tools in the installed binary.
...
cipherroute --robot sync 9router --dry-run
cipherroute --robot sync omniroute --dry-run

# Apply
cipherroute sync 9router
cipherroute sync omniroute

# Remove entries we previously synced that are no longer in the upstream
cipherroute sync 9router --prune
```
...
| Symptom | Fix |
|---|---|
| `EADDRINUSE :4623` | `cipherroute server stop` then restart, or `cipherroute --port 4624 server start --detach`. |
| `401 on /v1/*` | Wrong bearer; re-issue with `cipherroute key add` or fall back to the admin key from `server init`. |
...
| `npm install -g @openprx/cipherroute` → `E404` | npm publish path is currently disabled (registry stuck at 0.1.1). Use the curl installer (1a) on Linux/macOS or the PowerShell installer (1b) on Windows. |
...
| `cipherroute doctor` reports `server not running` | `cipherroute server start --detach --no-open`. |
...
| `CIPHERROUTE_DATA_DIR` / `DATA_DIR` | `~/.cipherroute` | Where `db.json`, `usage.json`, `log.txt` live. |
| `PORT` | `4623` | HTTP listen port. |
| `HOSTNAME` | `127.0.0.1` | Bind host. `0.0.0.0` exposes on LAN. |
| `INITIAL_PASSWORD` | _random, generated once_ | First-login password (replaced on first save). If unset, a random password is minted at first boot and printed in the startup banner; recover with `cipherroute auth reset-password --show`. |
| `JWT_SECRET` | `cipherroute-default-secret-change-me` | **Change in any non-throwaway deploy.** |
...
- The user has cipherroute running and wants help debugging combo dispatch — use `.agents/skills/testing-combo-fallback/SKILL.md` instead.
...
- The user explicitly wants to `--from-source` build cipherroute from scratch — follow the README's "Build from source" section; this skill optimizes for the prebuilt-binary path.

## See also

- Full README: https://github.com/quangdang46/cipherroute
- CLI reference: `cipherroute --help` and `cipherroute <command> --help`
- Schema introspection: `cipherroute schema list`
