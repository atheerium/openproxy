...
mod tests {
    ...
    fn compress_messages_accounts_for_image_blocks() {
        ...
        assert!(stats.bytes_before > 0, "image bytes should be counted in bytes_before");
        assert_eq!(stats.bytes_before, stats.bytes_after, "image bytes pass through unchanged");
    }
    ...
    fn compress_messages_skips_image_only_messages() {
        ...
        assert_eq!(stats.hits.len(), 0, "no text compression hits for image-only message");
    }
}
