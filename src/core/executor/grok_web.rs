...
//! Perplexity Web executor — `www.perplexity.ai/rest/sse/perplexity_ask`
//! (see the `PerplexityWebExecutor` section below).
...
///
/// Named `parse_pplx_messages` (not `parse_openai_messages`) to coexist with
/// the grok-web executor's `parse_openai_messages(&[Value]) -> String`.
fn parse_pplx_messages(messages: &Value) -> (String, Vec<(String, String)>, String) {
...
impl PerplexityWebExecutor {
    ...
    pub async fn execute_request(
        ...
        let (system_msg, history, current_msg) = parse_pplx_messages(&messages.unwrap());
...
mod tests {
    ...
    fn test_pplx_parse_messages() {
        ...
        let (system_msg, history, current_msg) = parse_pplx_messages(&messages);
