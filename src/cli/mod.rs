...
pub struct Cli {
    ...
    /// Falls back to $DATA_DIR or ~/.cipherroute.
    #[arg(long, env = "DATA_DIR", global = true)]
    pub data_dir: Option<PathBuf>,

    /// Emit a stable JSON envelope (`cipherroute.v1.*`) to stdout for agents.
    ...
    /// Optional config profile (from ~/.config/cipherroute/config.toml).
...
pub enum ServerCmd {
    ...
    Start {
        ...
        /// and SKILL.md show `cipherroute server start --detach --no-open`.
        #[arg(long)]
        no_open: bool,
    },
...
#[derive(Debug, Clone, Subcommand)]
pub enum AuthCmd {
    /// Save credentials for a remote cipherroute server as a profile.
    Login {
        /// Base URL of the remote server, e.g. https://op.example.com:4623
        #[arg(long)]
        url: String,
        /// API key generated by `cipherroute server init` on the remote server.
        #[arg(long)]
        api_key: String,
...
pub enum SchemaCmd {
    ...
    /// `cipherroute.v1.*` envelopes are declared **stable** (additive-only).
    Stability,
}
        ...
        KeyCmd::Add {
            ...
            let secret = if auto {
                ...
                let exit = output::emit_error(
                    ctx,
                    "usage",
                    "missing key secret. Pass a value positionally (`cipherroute key add <NAME> <SECRET>`) or use `--auto` to generate one.",
                )?;
                std::process::exit(exit);
            } else {
                key
            ...
            if ctx.is_robot() {
                ...
                output::emit_robot("cipherroute.v1.key.add", payload)?;
            } else if json {
...
async fn run_route(
    ...
    if let (Some(model_str), None) = (&model, &combo) {
        ...
        eprintln!("Usage: cipherroute route --model cc/claude-opus-5 --prompt 'hello'");
        eprintln!("   or: cipherroute route --combo default --prompt 'hello'");
        std::process::exit(1);
    }
}
