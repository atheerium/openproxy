...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }: CodexToolCardProps): React.ReactNode {
  ...
  const handleApplySettings = async (): Promise<void> => {
    ...
    setMessage(null);
    try {
      // Use sk_cipherroute for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = (): Array<{ filename: string; content: string }> => {
    ...
    const configContent = `# CipherRoute Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "cipherroute"
      ...
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <img src="/providers/codex.svg" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          </div>
